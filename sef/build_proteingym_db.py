#!/usr/bin/env python3
"""Build proteingym_data.db from source files.

Constructs a single SQLite database giving the strategy script access to
pre-computed SOTA model predictions, per-residue structure data, and MSA
statistics — without any ground-truth labels.

Sources:
    - DMS_substitutions.csv    -> protein_info (metadata + MSA stats)
    - protein_structures.db    -> has_structure, pdb_file, residue_structure
    - $PROTEINGYM_SCORES_DIR/*.csv  -> model_scores (VenusREM, S3F_MSA, ESM2_15B)

Output:
    $PROTEINGYM_DB_OUTPUT (the canonical DB)
    data/proteingym_data.db                          (symlink to the above)
...
Usage:
    python3 scripts/build_proteingym_db.py
"""
...
REFERENCE_FILE = os.environ.get(
    "PROTEINGYM_REFERENCE",
    os.path.join(DATA_DIR, "DMS_substitutions.csv"),
)
STRUCTURE_DB = os.environ.get(
    "PROTEINGYM_STRUCTURE_DB",
    os.path.join(DATA_DIR, "protein_structures.db"),
)
SCORES_DIR = os.environ.get(
    "PROTEINGYM_SCORES_DIR",
    os.path.join(DATA_DIR, "model_scores"),
)
DB_PATH = os.environ.get(
    "PROTEINGYM_DB_OUTPUT",
    os.path.join(DATA_DIR, "proteingym_data.db"),
)
SYMLINK_PATH = os.path.join(DATA_DIR, "proteingym_data.db")
...
def create_symlink():
    """Point data/proteingym_data.db at the canonical DB."""
    os.makedirs(DATA_DIR, exist_ok=True)
    if os.path.islink(SYMLINK_PATH) or os.path.exists(SYMLINK_PATH):
        if os.path.islink(SYMLINK_PATH) or os.path.isfile(SYMLINK_PATH):
            os.remove(SYMLINK_PATH)
        else:
            log(f"WARNING: {SYMLINK_PATH} exists as a directory — not removing")
            return
    os.symlink(DB_PATH, SYMLINK_PATH)
    log(f"Symlink: {SYMLINK_PATH} -> {DB_PATH}")
